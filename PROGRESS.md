...
- Push CI exposed two environment assumptions: browser-backed unit tests ran before Chromium installation, and E2E storage used macOS-only `/private/tmp`. The workflow now installs the pinned browser before `npm test`, while Playwright uses portable `/tmp`; neither correction changes product runtime behavior.
