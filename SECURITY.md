...
Useful interface semantics are not treated as form values. The Training compiler payload may contain only bounded, classified control names, associated labels, resolved ARIA labels, generic placeholders, button/link/option text, structural headings, control state, geometry, and DOM/visual relations. It excludes arbitrary page copy and hidden controls. Obvious token-, URL-, email-, UUID-, and long-number-like semantic strings are replaced with `[REDACTED]`. Because no heuristic can prove that every label is non-sensitive, Studio displays the exact payload and the backend requires an explicit human confirmation tied to its SHA-256 before compilation.
